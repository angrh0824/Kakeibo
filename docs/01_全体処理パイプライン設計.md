...
> **現行実装との対応**: この文書は将来構成を含む設計案です。現在は静的HTML/Vanilla JS + FastAPI + OpenRouter + Firestore + 非公開Cloud Storageで家族共有運用します。PostgreSQL、Redis/Celery、独立OCR、Next.js/WebSocketは未導入です。現行仕様は`00_現行実装仕様.md`を参照してください。
